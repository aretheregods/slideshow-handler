...
/**
 * Parses the properties of a shape into a schema-compliant object.
 * @param {Element} shapeNode - The shape's XML node.
 * @param {Object} slideContext - The context of the slide.
 * @returns {Object} The parsed shape properties, compliant with the defined schemas.
 */
export function parseShapePropertiesV2(shapeNode, slideContext) {
    const spPrNode = shapeNode.getElementsByTagNameNS(PML_NS, 'spPr')[0];
    if (!spPrNode) return {};
    ...
    // Fill Parsing (adapted for new schema)
    let fillNode = null;
    const fillTypes = ['solidFill', 'gradFill', 'noFill', 'blipFill', 'pattFill', 'grpFill'];
    for (const child of spPrNode.children) {
        if (fillTypes.includes(child.localName)) {
    ...
    if (fillNode) {
        if (fillNode.localName === 'noFill') {
            ...
            const colorObj = ColorParser.parseColor(fillNode);
            if (colorObj) properties.fill = { type: 'solid', color: colorObj };
        } else if (fillNode.localName === 'gradFill') {
            properties.fill = parseGradientFill(fillNode, slideContext);
        }
        // TODO: Add blipFill, pattFill, grpFill
    } else {
        // Theme fill logic
        const styleNode = shapeNode.getElementsByTagNameNS(PML_NS, 'style')[0];
        if (styleNode) {
            ...
            if (fillRefNode) {
                ...
                if (idx > 0 && slideContext.theme.formatScheme.fills[idx - 1]) {
                    ...
                    let colorObj = colorOverride || themeFill.color;
                    if (themeFill.type === 'solid') {
                        properties.fill = { type: 'solid', color: colorObj };
                    }
                } else if (colorOverride) {
                    properties.fill = { type: 'solid', color: colorOverride };
                }
            }
        }
    }

    // Border Parsing (from ln node)
    const lnNode = spPrNode.getElementsByTagNameNS(DML_NS, 'ln')[0];
    if (lnNode) {
        const lineProps = parseLineProperties(lnNode, slideContext);
        if (lineProps) {
            properties.border = {
                color: lineProps.color,
                width: lineProps.width,
                style: lineProps.dash && lineProps.dash.length > 0 ? 'dashed' : 'solid', // Simplified
                cap: lineProps.cap,
                join: lineProps.join,
            };
        }
    ...
    // Effect & Shadow Parsing
    const effectLstNode = spPrNode.getElementsByTagNameNS(DML_NS, 'effectLst')[0];
    if (effectLstNode) {
        const outerShdwNode = effectLstNode.getElementsByTagNameNS(DML_NS, 'outerShdw')[0];
        if (outerShdwNode) {
            const blur = parseInt(outerShdwNode.getAttribute('blurRad') || '0') / EMU_PER_PIXEL;
            const dist = parseInt(outerShdwNode.getAttribute('dist') || '0') / EMU_PER_PIXEL;
            const angle = parseInt(outerShdwNode.getAttribute('dir') || '0') / 60000;
            const color = ColorParser.parseColor(outerShdwNode);

            const rad = angle * (Math.PI / 180);
            const offsetX = dist * Math.cos(rad);
            const offsetY = dist * Math.sin(rad);
        ...
        // TODO: Add other effects to the effects array
    }
